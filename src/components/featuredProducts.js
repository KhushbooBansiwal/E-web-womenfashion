...
const FeaturedProducts = () => {
  const [added, setAdded] = useState([false, false, false, false]); // Array for tracking added state of each product
    ...
    <div className="container mx-auto py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {[img1, img2, img3, img4].map((img, index) => (
            <div key={index} className="relative bg-white p-6 rounded-lg shadow-lg border-4 border-green-100 transition-transform hover:scale-105">
            <img
              src={img}
              alt={`Product ${index + 1}`}
            ...
            <div className="text-center">
              ...
              <button
                ...
                className={`add-to-cart-btn text-white py-2 px-6 rounded-full flex items-center justify-center space-x-2 transition-transform duration-300 ease-in-out ${
                  added[index] ? 'bg-gray-500 cursor-not-allowed' : 'bg-green-700 hover:bg-green-600 hover:scale-105'
                }`}
                disabled={added[index]}
              >
                <FaShoppingCart />
                <span>{added[index] ? 'Added to Cart' : 'Add to Cart'}</span>
              </button>
            </div>
          </div>
        ))}
